...
let slider_excursion = new Swiper('.slider-excursion__slides', {
//	effect: 'fade',
	// autoplay: {
	// 	delay: 5000,
	// 	disableOnInteraction: false,
	...
	// Dotts
	pagination: {
		el: '.slider-excursion__dotts',
		clickable: true,
	},

	navigation: {
		nextEl: '.slider-excursion .slider-excursion__arrow_next',
		prevEl: '.slider-excursion .slider-excursion__arrow_prev',
	},
...
let slider_text_excursion = new Swiper('.slider-text-excursion__slides', {
	observer: true,
	observeParents: true,
	slidesPerView: 1,
	spaceBetween: 0,
...
slider_text_excursion.controller.control = slider_excursion;
slider_excursion.controller.control = slider_text_excursion;
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function formValidate(form) {
		...
		for (let index = 0; index < formReq.length; index++) {
			const input = formReq[index]; //получили конкретное поле ввода (значение)
...
window.onload = function () {
	...
	if (animItems.length > 0) {
		...
		function animOnScroll() {
			for (let index = 0; index < animItems.length; index++) {
				...
				if(animItem.classList.contains('_anim-item-m')) {//если есть модификация
					// if ((pageYOffset > animItemOffset - animItemPoint) && pageYOffset < (animItemOffset + animItemHeight +(animItemHeight - animItemPoint))) {
					// 	animItem.classList.add('_active');
					// } else {
					// 	animItem.classList.remove('_active');
					// }
					
				}else{//если нет модификации(дефолтная)
	...
	function headerActive (){
		...
		if (pixelsOS > 88 && pixelsOS > scrollPrev){
	...
	const mobileConfigArray = document.querySelectorAll('._mobile-config');
	if (mobileConfigArray.length > 0) {
		for (let index = 0; index < mobileConfigArray.length; index++) {
			const mobileCItem = mobileConfigArray[index];
			if (isMobile.any()){
				mobileCItem.classList.add('_mobile-config-active');
			}else{
				mobileCItem.classList.remove('_mobile-config-active');
...
//======================================================click
	...
	document.addEventListener("click", documentActions);
		...
		function documentActions(e) {
			...
			if (window.innerWidth < 768 && isMobile.any()){//spoilerforabout
				if(targetElement.classList.contains('about__more')){
					targetElement.closest('.about__images').classList.toggle('_hover');
				}
			}

			if (window.innerWidth > 768 && isMobile.any()){
				if(targetElement.classList.contains('arrow-menu')){
					targetElement.closest('.item-menu').classList.toggle('_hover');
				}
